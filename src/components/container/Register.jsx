...
const Register = () => {
    ...
    const validateSchema = Yup.object().shape({
        ...
        password: Yup.string()  // para testear lo dejare string, para la entrega cambiar a password
            .required("Password is Required").min(4, "min 4 caracteres").max(8, "max 8 caracteres"),
    ...
    return (
        <form onSubmit={handleSubmit} className="form-our">
            ...
            <div className='form-component'>
                <label htmlFor='confirmPass' className='form-component-label'>Confirm password</label>
                <input
                    name='confirmPass'
                    ...
                    onBlur={handleBlur}
                />
                {errors.confirmPass && touched.confirmPass && (<div className='form-component-errormesage '>{errors.confirmPass}</div>)}
            </div>
            <div className='flex flex-col space-y-1 gap-1 w-full items-center p-2'>
                 <button type='submit' className='btn-register' disabled={(errors.email || errors.password || errors.password) ? true : false}>Register {/*<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 inline-flex">
                    <path d="M6.25 6.375a4.125 4.125 0 118.25 0 4.125 4.125 0 01-8.25 0zM3.25 19.125a7.125 7.125 0 0114.25 0v.003l-.001.119a.75.75 0 01-.363.63 13.067 13.067 0 01-6.761 1.873c-2.472 0-4.786-.684-6.76-1.873a.75.75 0 01-.364-.63l-.001-.122zM19.75 7.5a.75.75 0 00-1.5 0v2.25H16a.75.75 0 000 1.5h2.25v2.25a.75.75 0 001.5 0v-2.25H22a.75.75 0 000-1.5h-2.25V7.5z" />
                </svg>*/}</button> 
                <>¿Do you have account?</>
                <button className='btn-login' onClick={() => navigate(`/login`)}>Login
